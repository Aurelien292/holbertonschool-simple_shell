...
void read_input(char **input) {
	size_t len = 0;  /* Taille initiale allouée pour getline */
	ssize_t nread;
printf("MonShell> ");
/* Appel à getline. Cela alloue et ajuste dynamiquement la mémoire nécessaire pour stocker l'entrée.*/
    nread = getline(input, &len, stdin);
if (nread == -1) {  /* Vérifier les erreurs de lecture */
	clear_history();
